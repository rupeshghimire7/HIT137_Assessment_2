...
def test_ask_for_shift_accepts_valid_number(monkeypatch):
    monkeypatch.setattr("builtins.input", lambda message: "5")

    result = ask_for_shift("Enter shift: ")

    assert result == 5


def test_ask_for_shift_accepts_zero(monkeypatch):
    monkeypatch.setattr("builtins.input", lambda message: "0")
...
def test_ask_for_shift_rejects_text(monkeypatch):
    ...
    monkeypatch.setattr("builtins.input", lambda message: next(inputs))
...
def test_main_completes_encryption_and_decryption(monkeypatch, tmp_path, capsys):
    ...
    (tmp_path / "text_files" / "raw_text.txt").write_text(
    ...
    encrypted_file = tmp_path / "text_files" / "encrypted_text.txt"

    decrypted_file = tmp_path / "text_files" / "decrypted_text.txt"
...
def test_main_handles_missing_raw_file(monkeypatch, tmp_path, capsys):
    monkeypatch.chdir(tmp_path)
...
def test_main_handles_empty_raw_file(monkeypatch, tmp_path, capsys):
    monkeypatch.chdir(tmp_path)

    (tmp_path / "text_files").mkdir()

    (tmp_path / "text_files" / "raw_text.txt").write_text("", encoding="utf-8")
